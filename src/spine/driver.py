...
class Driver:
    ...
    def initialize_base(
        ...
        metrics : Mapping[str, Any] | None, optional
            Metric-logging backend configuration. Exactly one of the following
            keys may be present (they are mutually exclusive):

            ``tensorboard`` : bool | Mapping[str, Any]
                TensorBoard configuration forwarded to ``LogManager``.
            ``wandb`` : bool | Mapping[str, Any]
                Weights & Biases configuration forwarded to ``LogManager``.
                Supported keys: ``project``, ``entity``, ``name``, ``tags``,
                ``notes``, ``group``, ``mode``, ``run_dir``.

            ``None`` or an empty mapping disables metric logging (CSV only).
        ...
        self.logmanager_cfg = dict(metrics) if metrics else {}
    ...
    def initialize_log(self) -> None:
        ...
        # Initialize CSV and optional metric-logging backend (TensorBoard or W&B)
    ...
    @property
    def iter_per_epoch(self) -> int:
        """Number of iterations per epoch, delegated to the IO manager."""
        return self.io.iter_per_epoch
